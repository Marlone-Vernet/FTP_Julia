...
@everywhere begin
    ...
    function extract_number(filenames::String)
        # Match the pattern that captures the number before the file extension
        MAT = match(r"(\d+)\.tiff$", filenames) # # # # # # # # attention parfois .tiff ou .tif !!!
        # print(MAT)
    ...
    name_gen = "20Hz"
    folder = "C:/Users/turbulence/Desktop/Data_images/150425/" 
    folder_save = "E:/DATA_FTP/150425/" 
    folder_def = "$(name_gen)"
    folder_ref = "ref"
    folder_gray = "gray"
    folder_map = "h_map_$(name_gen)"
    ...
    name_gray_1 = element_in_gray[50]
    name_ref_1 = element_in_ref[50]

    full_gray_1 = joinpath(folder,folder_gray,name_gray_1)
    full_ref_1 = joinpath(folder,folder_ref,name_ref_1)

    gray_m = channelview( FileIO.load(full_gray_1) )'
    reference =  channelview( FileIO.load(full_ref_1) )'

    """ average over set of grays and refs """

    #for m=1:100
    #    name_gray = element_in_gray[m]
    #    name_ref = element_in_ref[m]

    #    full_gray = joinpath(folder,folder_gray,name_gray)
    #    full_ref = joinpath(folder,folder_ref,name_ref)

    #    gray_m += channelview( FileIO.load(full_gray) )'./100
    #    reference += channelview( FileIO.load(full_ref) )'./100
    #end 
    resfactor_test = mean(reference)/mean(gray_m)
    ref_m_gray_test = reference .- resfactor_test.*gray_m
    ...
    function parallel_analysis(start,stop,folder,folder_save,folder_def,folder_map,resfactor,gray_,gaussian_filter,phase_ref, L, D, pspp)
        ...
        filtre_def = Filtre(element_in_def) #
        sorted_def = sort(filtre_def, by=extract_number)
